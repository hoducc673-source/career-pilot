...
- 浏览 11 个真实岗位样本并识别硬门槛
- 粘贴任意招聘 JD，在本地提取职责、要求和可验证硬门槛
- 临时解析脱敏 DOCX 简历，生成证据匹配报告
- 记录收藏、准备、投递、面试和 Offer 进度，用 CSV 备份恢复
- 基于 JD 和脱敏简历证据生成面试问题、回答结构与诚信边界
...
1. 打开“岗位雷达”，选择“粘贴新 JD”。
2. 粘贴完整的岗位职责与任职要求；公司、岗位、城市和原始链接可手动补充。
3. 点击“本地解析这份 JD”。这一步不调用 DeepSeek，也不将 JD 写入项目。
4. 解析结果会在本次网页会话中加入“简历匹配”的目标岗位列表。

粘贴的 JD 只有在用户上传脱敏简历、勾选同意并点击“生成匹配报告”后，才会与简历证据一起发送给 DeepSeek。

### 管理投递进度

1. 打开“投递看板”，从岗位雷达快速加入岗位，或手动新增。
2. 为每条记录填写状态、截止日期、简历版本和唯一的下一步行动。
3. 开始面试或获得 Offer 后，在原记录中更新状态。
4. 离开页面前下载 `careerpilot-applications.csv`；下次用“导入恢复”重建看板。

公开部署不把投递记录写入服务器数据库。这避免了不同访客互相看到求职数据；CSV 是用户自己保管的可携带备份。

### 生成面试准备题包

1. 打开“面试准备”，选择目标岗位。
2. 上传脱敏 DOCX 简历，预览将发送的证据文本。
3. 勾选隐私确认，点击“生成面试题包”。
4. 按“问题 → 证据 → 结构 → 边界”练习，并下载 Markdown 题包。
...
网页版包含职业方向探索、真实岗位筛选、脱敏简历匹配、投递看板、面试准备和带引用知识库问答。简历文件只在临时目录解析；只有在用户明确勾选确认后，脱敏文本才会发送给 DeepSeek。
